...
`v1.0.1` 版本重點優化安裝體驗：使用者只需要輸入校園網使用者名稱、密碼，以及一條從瀏覽器複製的完整 ePortal 登入長網址。詳細診斷資訊會寫入 `logs\shu-netauth.log`，不會堆在安裝視窗裡。
...
用於後續安全策略組。在 `v1.0.1` 中，該介面不會強制執行 host pinning、public key pinning 或憑據提交端點限制。
